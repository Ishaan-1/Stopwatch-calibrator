...
void drawWatch(uint32_t totalMs) {
    ...
    ILI9341_Draw_Text("Aadhikal Technologies", 0,0, WHITE,2,NAVY);

    char timeBuf[20];
    snprintf(timeBuf, sizeof(timeBuf), "%lu:%02lu:%02lu.%03lu",
             (unsigned long)hours,
             (unsigned long)mins,
             (unsigned long)secs,
             (unsigned long)millis);
    ILI9341_Draw_Text(timeBuf, 0,90, WHITE,4,BLACK);

        char* status =
            (swState == SW_RUNNING) ? " RUNNING " :
            (swState == SW_PAUSED)  ? " PAUSED  "  : " READY   ";
        ILI9341_Draw_Text(status, 10,200, BLACK,3,YELLOW);
    }
...
int main(void)
{
  ...
  /* Infinite loop */
    ...
    while (1)
    {
        ...
        // If the watch is paused or reset, redraw it once
        else if (refreshDisplay) {
            refreshDisplay = 0; // Clear flag
            drawWatch(elapsedMs);
        }

        HAL_Delay(33); // Reduced delay slightly (~30 FPS) for smoother updates
      /* USER CODE END WHILE */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, RES_Pin|DC_Pin|CS_Pin, GPIO_PIN_SET);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if(GPIO_Pin == GPIO_PIN_10)
  {
      ...
      refreshDisplay = 1; // Trigger a redraw in the main loop
  }
